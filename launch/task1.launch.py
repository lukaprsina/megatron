...
def generate_launch_description():
    ...
    args = [
        DeclareLaunchArgument(
            "world", default_value="task1", description="Gazebo world name"
        ),
        DeclareLaunchArgument(
            "map",
    ...
    sim_nav = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            PathJoinSubstitution(
                [pkg_megatron, "launch", "sim_turtlebot_nav.launch.py"]
            )
        ),
        launch_arguments=[
            ...
            (
                "nav2_config",
                PathJoinSubstitution([pkg_megatron, "config", "nav2.yaml"]),
            ),
        ],
    )
