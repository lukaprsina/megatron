...
class YellowAvoider(Node):
    ...
    def _update(self):
        ...
        roi_l = mask[dt:db, xl_l:xl_r] if xl_r > xl_l else np.zeros((db - dt, 1), dtype=np.uint8)
        roi_r = mask[dt:db, xr_l:xr_r] if xr_r > xr_l else np.zeros((db - dt, 1), dtype=np.uint8)
        ...
        if cast(bool, self.get_parameter("publish_debug_image").value):
            self._publish_debug(
                img, mask,
                xc_l, xc_r, xl_l, xl_r, xr_l, xr_r, dt, db,
                danger_px, c_reaches, l_reaches, r_reaches,
            )
    ...
    def _publish_debug(
        ...
        col_l = grn if self._safe_side == "L" and not l_reaches else (
            red if self._safe_side == "L" else dim
        )
        cv2.rectangle(debug, (xl_l, dt), (xl_r, db), col_l, 2)
        col_r = grn if self._safe_side == "R" and not r_reaches else (
            red if self._safe_side == "R" else dim
        )
        ...
        if self._avoider_state == "STEERING":
            col = (255, 140, 0)
            self._label(
                debug, f"STEER {self._steer_dir:+d}", (8, 40), col
            )
