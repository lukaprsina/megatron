...
I am Luka, we are working on Task 1, our project is called Megatron. A few hours ago me and a teammate split branches, help us make a synthesis of the controller.py, and even surpass it. I used an LLM agent to fix a few bugs in the controller while I was working on face and ring detection. I am not sure if they even work, it could all be bs. ` _requeue_if_not_pending` definitely doesn't work, however, the `self.nav_in_flight` thing is necessary I think. Can you see how to do it better? My teammate Blaž's job was to fix the controller, and he implemented a task_queue, works well, but there are some problems (he also used the copilot agent). Here are my thoughts:

1. there are `_compute_path_length_sync` and the async version `_compute_path_length_to`. are they both necessary? it just gets the real length, and the euclidian is the fallback. the sync version actually starts a MissionController for every object detection (objects are faces and rings), that is too heavy right? we can replace the sync version with async and a semaphore/flag right? or do you have a better idea? we can scrap this and just use euclidian distance, but that means it will sometimes go the long way around a long wall, while a computed path won't. the map is small, so it doesn't really matter.
2. is the logic to "insert" objects in-between waypoint okay? what if we used a priority queue? is the priority just the calculated distance or something more complex? or maybe just sort the list after any object is detected. any other ideas?
3. inside `_schedule_detection`, more concretely `_compute_approach_pose`, we don't check the costmap. if a ring is close to a wall, the approach point can be inside the lethal obstacle. see the attached picture, there is a ring that has a perpendicular wall beside it, and the straight-on approach point is inside the lethal obstacle. what's more, if the local cost map updates mid-approach, it can trap the robot. that also happens. do you think we should subscribe to either `/global_costmap/costmap` or `/local_costmap/costmap`? i think it's worth the complexity. we can check points left and right (30cm) from the approach point too. if all of them fail, check `self.approach_retry_offset` further away and so on. be aware that we already have this kind of logic, but it's for when the bot is already at the point, and aborts. but it doesn't even work, in both controllers.

Anything else? Think deeply about this, prefer to simplify. Search the web if you are unsure about anything, don't write the final code, just talk to me.
