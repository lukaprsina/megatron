...
class ArmMoverAction(Node):
    def __init__(self):
        ...
        self.arm_command_sub = self.create_subscription(
            String, "/arm_command", self.arm_command_callback, 1
        )
        self.arm_position_client = ActionClient(
        ...
        self.get_logger().info(
            "Initialized the Arm Mover node! Waiting for commands..."
        )
    ...
    def set_arm_position(self, command_string):
        ...
        while not self.arm_position_client.wait_for_server(timeout_sec=1.0):
            self.get_logger().info(
                "'Arm controller' action server not available, waiting..."
            )

        point = JointTrajectoryPoint()

        command = self.arm_poses[
            command_string.split(":")[0]
        ]  # Only beacause we want to detect manual setting of position
        # self.get_logger().info(f"Recieved command {command}")
        if command is None:
            self.get_logger().info(
                f"Recieved command MANUAL command {command_string.split(':')[1]}"
            )
            point.positions = eval(command_string.split(":")[1])
        else:
            point.positions = command
        point.time_from_start = rclpy.duration.Duration(seconds=3.0).to_msg()

        goal_msg = FollowJointTrajectory.Goal()
        goal_msg.goal_time_tolerance = rclpy.duration.Duration(seconds=3.0).to_msg()
        goal_msg.trajectory.joint_names = self.joint_names
        goal_msg.trajectory.points = [point]

        self.get_logger().info(
            f"Sending a goal to the action server, position is {command}"
        )
    ...
    def get_result_callback(self, future):
        ...
        if status != GoalStatus.STATUS_SUCCEEDED:
            ...
            self.get_logger().info("Arm controller says GOAL REACHED.")
    ...
    def timer_callback(self):
        if self.new_command_arrived and not self.executing_command:
            ...
            self.get_logger().info(
                f"Will set a new position for the arm joints: {self.current_command}"
            )
    ...
    def arm_command_callback(self, msg):
        ...
        command_test = (
            msg.data.strip().lower().split(":")[0]
        )  # Split is only needed to be able to set a position manually
        ...
        self.get_logger().info(
            f"Got a new command for the arm configuration: {command_string}"
        )
