...
def _overlay_label(
    ...
    overlay = image.copy()
    cv2.rectangle(
        overlay, (x, y), (x + tw + 2 * pad, y + th + 2 * pad + baseline), (0, 0, 0), -1
    )
...
class PerceptionVisualizer(Node):
    ...
    def __init__(self) -> None:
        ...
        self.panel_height = (
            self.get_parameter("panel_height").get_parameter_value().integer_value
        )
        ...
        self.create_subscription(
            Image, "/ring_debug/ellipses", self._rd_ellipses_cb, 10
        )
    ...
    def _tick(self) -> None:
        ...
        try:
            self.rviz_pub.publish(
                self.bridge.cv2_to_imgmsg(rviz_canvas, encoding="bgr8")
            )
            self.image_pub.publish(
                self.bridge.cv2_to_imgmsg(debug_canvas, encoding="bgr8")
            )
        except CvBridgeError as exc:
